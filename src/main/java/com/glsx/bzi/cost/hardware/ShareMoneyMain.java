...
public class ShareMoneyMain {
    ...
    private static void computeShareMoney(Connection mysqlConn, String type1, String type2, int isReturn) {
        ...
        //String sql = "SELECT sn," + type2 + ",settle_merchant,period,price from t_hardware_cost " +
        //        "where " + type2 + " !='null' and  mer_type='" + type1 + "'" ;//+ " and " + type2 + "<'2021-01-01'";
        String sql = "";
        if(isReturn == 0) {
            sql = "SELECT sn," + type2 + ",settle_merchant,pk_period,hw_cost from dj_2021_settle_details_agg " +
                    "where " + type2 + " !='null'  and hw_cost is not null and  channel in " + type1;// + " and " + type2 + "<'2021-01-01'";
        }
        else {
            sql = "SELECT sn," + type2 + ",settle_merchant,pk_period,hw_cost from dj_2021_settle_details_return " +
                    "where " + type2 + " !='null'  and hw_cost is not null and  channel in " + type1;// + " and " + type2 + "<'2021-01-01'";
        }
...
//                    else if(type.equals("直销"))
//                        monTime = DateUtils.convTime1(time);
//                    else if (type.equals("渠道"))
//                        monTime = DateUtils.convTime1(time);
//                    else
//                        System.out.println("没找到对应的类型匹配");
